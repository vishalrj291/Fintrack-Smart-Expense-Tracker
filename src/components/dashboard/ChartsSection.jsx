...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="glass-card" style={{ padding: '12px', borderRadius: '12px', border: '1px solid #1f1f35', fontSize: '12px' }}>
        <p style={{ color: '#9ca3af', marginBottom: '8px', fontWeight: 500 }}>{label}</p>
        {payload.map((p, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: p.color }} />
            <span style={{ color: '#9ca3af', textTransform: 'capitalize' }}>{p.dataKey}:</span>
            <span style={{ color: 'white', fontWeight: 600, fontFamily: 'JetBrains Mono, monospace' }}>${p.value?.toLocaleString()}</span>
          </div>
        ))}
      </div>
    );
    ...
    <div className="glass-card" style={{ padding: '24px', borderRadius: '16px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '16px', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ fontSize: '12px', color: '#6b7280' }}>6-month performance overview</p>
        </div>
          ...
          {tabs.map(tab => (
            <button
              ...
              style={{
                ...
                color: activeTab === tab ? 'white' : '#6b7280', background: 'none', border: 'none', cursor: 'pointer',
              }}
            >
              {activeTab === tab && (
                <motion.div
        ...
        {activeTab === 'Overview' && (
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={monthlySpending}>
              ...
              <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '16px' }} formatter={(v) => <span style={{ color: '#9ca3af' }}>{v}</span>} />
          ...
          <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: '32px' }}>
            <div style={{ flexShrink: 0 }}>
              <ResponsiveContainer width={200} height={200}>
                <PieChart>
                  <Pie data={categorySpending} cx={100} cy={100} innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value">
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ background: '#13131f', border: '1px solid #1f1f35', borderRadius: '12px', color: '#fff', fontSize: '12px' }}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div style={{ flex: 1, minWidth: '180px' }}>
              {categorySpending.map((cat, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: '#9ca3af', flex: 1 }}>{cat.name}</span>
                  <div style={{ width: '80px', background: 'rgba(255,255,255,0.06)', borderRadius: '4px', height: '4px', overflow: 'hidden' }}>
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{ width: `${(cat.value / 7831) * 100}%` }}
                      transition={{ duration: 0.8, delay: i * 0.05 }}
                    />
                  </div>
                  <span style={{ fontSize: '12px', color: 'white', fontFamily: 'JetBrains Mono, monospace', width: '48px', textAlign: 'right' }}>${cat.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === 'Weekly' && (
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={spendingTrend}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Line type="monotone" dataKey="amount" stroke="#6366f1" strokeWidth={2.5} dot={{ fill: '#6366f1', strokeWidth: 2, r: 4 }} activeDot={{ r: 6, fill: '#8b5cf6' }} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </motion.div>
